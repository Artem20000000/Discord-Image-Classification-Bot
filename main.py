...
bot = telebot.TeleBot("7574128015:AAGaJYUkrgv3vkfhrXh0klEEySK66PNCpmA")
...
def send_photo(message):
    if not message.photo:
        return bot.send_message(message.chat.id,"вы забыли загрузить фотографии")
    file_info = bot.get_file(message.photo[-1].file_id)
    file_name = file_info.file_path.split("/")[-1]
    downolaled_file = bot.download_file(file_info.file_path)
    with open(file_name , "wb") as new_file:
        new_file.write(downolaled_file)
